...
export const getBasketPhonesWithCount = state => {
    ...
    // сравниваем id телефона с количеством каждого элемента
    // и фильтруем, оставляя при проходе только id, которые сейчас проверяются
    const phoneCount = id => R.compose(
        R.length,
        R.filter(basketId => R.equals(id, basketId))
    )(state.basket)
    // телефон, у которого будет новое поле count. И это поле будет высчитываться этой функцией phoneCount(phone.id)
